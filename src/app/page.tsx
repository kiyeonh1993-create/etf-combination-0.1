      ...
      <div className="max-w-3xl mx-auto space-y-10">
        ...
        <header className="text-center space-y-4 py-4 px-2">
          <div className="flex justify-center gap-2">
            <span className="bg-[#3182F6] text-white text-[10px] px-3 py-1 rounded-full font-black shadow-sm uppercase tracking-widest">v0.3.1 Pro</span>
          </div>
        ...
        {/* Future Value with Auto-scaling Text */}
        ...
        <section className="bg-white p-8 md:p-10 rounded-[40px] shadow-sm border border-[#F2F4F6] space-y-12">
          <div className="space-y-8">
            ...
            <div className="relative pt-2">
              ...
              />
              <div className="flex justify-between mt-5 text-[10px] md:text-[11px] font-black text-[#ADB5BD] uppercase tracking-tighter">
                <span className="text-[#10B981]">배당 집중형</span>
                <span>황금 밸런스</span>
          ...
          <div className="space-y-8 pt-6 border-t border-[#F2F4F6]">
            <div className="flex justify-between items-center">
              ...
              <div className="bg-[#F2F4F6] text-[#4E5968] px-5 py-2 rounded-2xl text-lg font-black italic">
                {investmentYears}년
              </div>
            </div>
            <input 
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white p-6 md:p-10 rounded-[40px] shadow-sm border border-[#F2F4F6] space-y-8 overflow-hidden">
            <h3 className="text-xl font-black tracking-tight flex items-center gap-2 px-2">
              <Activity className="text-[#3182F6] w-5 h-5" /> 수익률 vs 위험도
            </h3>
            <div className="h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 20, right: 30, bottom: 20, left: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#F2F4F6" />
                  <XAxis type="number" dataKey="risk" domain={[10, 25]} axisLine={false} tickLine={false} tick={{fill: '#ADB5BD', fontSize: 10}} label={{ value: '투자 위험(변동성)', position: 'insideBottom', offset: -10, fontSize: 9, fill: '#ADB5BD', fontWeight: 'bold' }} />
                  <YAxis type="number" dataKey="return" domain={[10, 20]} axisLine={false} tickLine={false} tick={{fill: '#ADB5BD', fontSize: 10}} label={{ value: '기대 수익률', angle: -90, position: 'insideLeft', fontSize: 9, fill: '#ADB5BD', fontWeight: 'bold' }} />
                  <ZAxis type="number" range={[100, 400]} />
                  <Tooltip cursor={{ strokeDasharray: '3 3' }} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 20px rgba(0,0,0,0.05)' }} formatter={(value) => [value + "%"]} />
                  <Scatter data={frontierData} fill="#E5E8EB" line={{ stroke: '#E5E8EB', strokeWidth: 2 }}>
                    {frontierData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.qqq === results.currentFrontierPoint.qqq ? '#3182F6' : '#E5E8EB'} />
                    ))}
                  </Scatter>
                  <Scatter data={[results.currentFrontierPoint]} fill="#3182F6" />
                </ScatterChart>
              </ResponsiveContainer>
            </div>
          </section>
          ...
          <section className="bg-white py-10 px-4 md:px-8 rounded-[40px] shadow-sm border border-[#F2F4F6] space-y-8 overflow-hidden">
            <h3 className="text-xl font-black tracking-tight px-4 md:px-6">성장 곡선 비교</h3>
            <div className="h-[360px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={results.chartData} margin={{ left: 0, right: 20, top: 10, bottom: 0 }}>
                  ...
                  <Legend verticalAlign="top" align="right" iconType="circle" iconSize={8} wrapperStyle={{ fontSize: '10px', fontWeight: 'bold', color: '#8B95A1', paddingBottom: '30px', paddingRight: '10px' }} />
            ...
            <div className="mx-4 md:mx-6 bg-[#F9FAFB] p-5 rounded-3xl flex items-start gap-3">
              ...
              <div className="text-[11px] text-[#4E5968] leading-relaxed font-medium">
                <p>• <strong className="text-[#3182F6]">파란 실선</strong>은 <strong className="text-[#3182F6]">내 조합</strong>의 성과입니다.</p>
        ...
        <footer className="text-center pt-10 pb-32 space-y-6">
          <p className="text-[11px] text-[#B0B8C1] leading-relaxed px-8">
            ...
            실제 투자 수익은 제반 비용 및 시장 상황에 따라 달라질 수 있습니다.
          </p>
        </footer>
      </div>
